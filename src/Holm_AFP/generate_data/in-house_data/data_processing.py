...
def process_taxonomy_line(line, data, selected_features):
    ...
    fields = line.split('\t')
    name = fields[0].split('|')[1]
    features = fields[2].strip().split(';') #NOTE this is 3 for the new cafa3 data, was 2 before
    data[name] = [feature for feature in features if feature in selected_features]
...
def ipscan_cover(ipr_dir):
    ...
    for t1, sequence in data.items():
        cover = set()
        for t2, feature in sequence.items():
            cover.update(list(range(feature[0][2], feature[0][3])))
        data[t1] = len(cover)
    return data
