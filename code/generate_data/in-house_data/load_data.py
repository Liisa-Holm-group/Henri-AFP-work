...
if sys.argv[-1] == '-p':
    ...
    # import data_processing_cafa_3_eval as data_processing
    # import dp_papu as data_processing

else:
    import data_processing as data_processing #NOTE
    # import data_processing_cafa as data_processing
...
def load_ipscan(path, sequences, pretrained, output_path):
    ...
    for jj in range(4):
        maxes = []
        for c in range(len(count_and_loc[1])):
            if len(count_and_loc[jj][c].values()) > 0:
                tt = min(list(count_and_loc[1][c].values()))
                if tt < 0:
                    bp()
                maxes.append(tt)
        res.append(maxes)
...
def load_targets(path, sequences):
    ...
    targets = data_processing.process_target_data(path, min_count=20) #NOTE change to 1 for new data
...
def load_taxonomy(path, sequences, pretrained, output_path):
    ...
    binarizer = MultiLabelBinarizer(sparse_output=True)
    features = data_processing.process_taxonomy_data(path, min_count=1) #NOTE this was 1 change to 0 fro new_cc data
...
class DatasetGenerator:
    ...
    def __init__(self, data_path, ontology, output_dir, pretrained=False):
        ...
        if not self.pretrained:
            targets = data_processing.process_target_data(self.target_path, min_count=20) #NOTE was 20
    ...
    # def generate_cafa3_targets(self):
    #     targets = data_processing.process_target_data(self.target_path, min_count=20)# NOTE was 20
    #     sequences = sorted(targets.keys())
    #     targets, names = load_targets(self.target_path, sequences)
    #     with open(f'{self.data_path}/{self.ontology}_eval_sequences.txt', 'r') as f:
    #         seq = f.readlines()
    #         eval_sequences = set([s[:-1] for s in seq])
    #     index = np.array([i for i, s in enumerate(sequences) if s in eval_sequences])
    #     joblib.dump([sequences[i] for i in index], f'{self.output_dir}/{self.ontology}_eval_sequences.joblib')
    #     joblib.dump(names, f'{self.output_dir}/{self.ontology}_eval_target_names.joblib')
    #     sp.save_npz(f'{self.output_dir}/{self.ontology}_eval_targets.npz', targets[index,:])
    #     pr.targets_to_cafa(f'{self.output_dir}', self.ontology)
    ...
    def generate_targets(self):
        if not self.pretrained:
            ...
            joblib.dump(self.sequences, f'{self.output_dir}/CAFA_{self.ontology}_sequences.joblib')
    ...
    def generate_features(self, feature_names, dataset_name):
        ...
        parallel = Parallel(n_jobs=1)#len(feature_names))
        ...
        # bp()
        # if not self.pretrained: #this is for cafa3
        #     # remove filtered sequences
        #     results = self.filter_sequences(results)
    ...
    def filter_sequences(self, results):
        counts = dict(zip(self.sequences, self.targets.sum(axis=1)))
        seqs = joblib.load(f'{self.output_dir}/filtered_sequences.joblib')
        index = []
        for s in seqs:
            try:
                index.append(s[0]) if counts[s[0]] < counts[s[1]] else index.append(s[1])
            except KeyError:
                continue
        # remove these sequences
        index2 = np.array([self.sequences.index(fi) for fi in index])
        final_sequences = np.setdiff1d(np.arange(results.shape[0]), index2)
        results = results.tocsr()[final_sequences,:]
        targets = self.targets.tocsr()[final_sequences,:]
        sequences = np.array(self.sequences)[final_sequences]
        sp.save_npz(f'{self.output_dir}/{self.ontology}_targets.npz', targets)
        joblib.dump(sequences, f'{self.output_dir}/{self.ontology}_sequences.joblib')
        return results
    ...
    def ipscan_eval(self):
        self.generate_features(['ipscan','taxonomy'], 'ipscan')

    def cafa3(self):
        # self.generate_cafa3_targets()
        self.generate_targets()
        self.generate_features(['ipscan','taxonomy', 'targetp', 'wps'], 'cafa3')

    def cafa3_eval_data(self):
        self.generate_targets()
        self.generate_features(['ipscan','taxonomy', 'targetp', 'wps'], 'cafa3_eval_data')

    def cafa3_only_ipscan(self):
        # self.generate_cafa3_targets()
        self.generate_targets()
        self.generate_features(['ipscan'], 'cafa3_only_ipscan')

    def cafa3_eval_data_only_ipscan(self):
        self.generate_targets()
        self.generate_features(['ipscan'], 'cafa3_eval_data_only_ipscan')
